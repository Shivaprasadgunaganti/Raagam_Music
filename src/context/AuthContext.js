  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // The homepage ("/") is public now, so a first-time / signed-out
        // visitor should behave exactly like guest mode immediately —
        // not after a follow-up effect/render. Setting this here means
        // `isGuest` is already correct in the SAME render pass where
        // `loading` flips to false, so there's no lag window where
